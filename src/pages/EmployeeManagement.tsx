					...
					<tbody
						...
						{doctorList != null &&
							doctorList
								...
								.map((doctor) => {
									// Create an object to store unique clinic names
									const uniqueClinicNames = {};
									// Iterate through the availableClinics of the doctor
									doctor.availableClinics.forEach((clinic) => {
										// Normalize the clinic name
										const clinicName = clinic.clinic.name.toLowerCase();
										// Store the clinic name as a key
										uniqueClinicNames[clinicName] = true;
									});
									...
									return (
										<tr key={doctor.id}>
											...
											<td>
												<ListGroup>
													{Object.keys(uniqueClinicNames).map(
														(clinicName, index) => (
